...
class LevelSystem(Cog):
    ...
    async def level(self, ctx, member: nextcord.Member = None):
        if not checkIfLevelSystemIsEnabled(ctx.guild):
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, getGuildLanguage(ctx.guild.id), "levelsysNotEnabled"))
        ...
        await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysUserLevel", member, user.level, user.xp, user.xp_needed, placing, len(allUsers), getLocale(self.bot, languageStrings, guildLocale, "yes" if top10 else "no")), thumbnail=member.display_avatar.url, color=member.color)
    ...
    async def leaderboard(self, ctx):
        ...
        if len(allUsers) == 0:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysNoUsers"))
        ...
        await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysLeaderboard", leaderboard))
    ...
    async def levelsystem(self, ctx, option: str = None, value: str = None):
        ...
        if option is None or option.lower() in {"help", "h", "info", "i", "settings", "s"} or not ctx.author.guild_permissions.manage_guild:
            ...
            message = await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysHelp", getPrefixFromDatabase(self.bot, ctx.message)[0]), view=view)
        ...
        if option.lower() in {"enable", "e", "on"}:
            if guild.enabled:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysAlreadyEnabled"))
            
            guild.enabled = True
            update("level_system", "enabled", "guild_id", [1, ctx.guild.id])
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysEnabled"))
        
        if option.lower() in {"disable", "d", "off"}:
            if not guild.enabled:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysAlreadyDisabled"))
            
            guild.enabled = False
            update("level_system", "enabled", "guild_id", [0, ctx.guild.id])
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysDisabled"))
            
        if option.lower() in {"xp", "exp", "experience"}:
            if value is None or not value.isdigit():
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysXpValue"))
            ...
            if int(value) == guild.xp:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysXpAlready", value))
            
            guild.xp = int(value)
            update("level_system", "xp", "guild_id", [int(value), ctx.guild.id])
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysXpCustom", value))
        
        if option.lower() in {"cooldown", "cd"}:
            if value is None or not value.isdigit():
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCooldownValue"))
            ...
            if int(value) == guild.cooldown:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCooldownAlready", value))
            
            guild.cooldown = int(value)
            update("level_system", "cooldown", "guild_id", [int(value), ctx.guild.id])
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCooldownCustom", value))
        
        if option.lower() in {"mention", "ping"}:
            if value is None:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMentionValue"))
            
            if value.lower() in {"enable", "e", "on"}:
                if guild.mention:
                    return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMentionAlreadyEnabled"))
                
                guild.mention = True
                update("level_system", "mention", "guild_id", [1, ctx.guild.id])
                return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMentionEnabled"))
            
            if value.lower() in {"disable", "d", "off"}:
                if not guild.mention:
                    return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMentionAlreadyDisabled"))
                
                guild.mention = False
                update("level_system", "mention", "guild_id", [0, ctx.guild.id])
                return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMentionDisabled"))
            
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMentionValue"))
        
        if option.lower() in {"current", "show"}:
            yes = getLocale(self.bot, languageStrings, guildLocale, "yes")
            no = getLocale(self.bot, languageStrings, guildLocale, "no")
            none = getLocale(self.bot, languageStrings, guildLocale, "none")
            enabled = yes if guild.enabled else no
            xp = str(guild.xp)
            cooldown = f"{str(guild.cooldown)} {getLocale(self.bot, languageStrings, guildLocale, 'seconds')}"
            ...
            return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCurrent", enabled, xp, cooldown, getPrefixFromDatabase(self.bot, ctx.message)[0], mention, channel, keysFromCustom_messages, roles, blacklist_channel, blacklist_roles), thumbnail=iconURL)
    ...
    async def levelsystem_message(self, ctx, *, message: str = None):
        ...
        if message is None:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMessageValue"))
        
        if message.lower() in {"current", "show"}:
            if guild.message == "none":
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMessageNone"))
            
            user: User = readUser(ctx.guild, ctx.author.id)
            
            return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMessageCurrent", guild.message.replace("\\n", "\n").format_map(safeDict(user_name=ctx.message.author.name, user_mention=ctx.message.author.mention, user_discriminator=ctx.message.author.discriminator, level=user.level, xp_needed=user.xp_needed, level_next=user.level + 1, role="{role}"))))
        
        if message.lower() in {"none", "off"}:
            guild.message = "none"
            update("level_system", "message", "guild_id", ["none", ctx.guild.id])
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMessageNone"))
        
        if message.lower() in {"default", "standard"}:
            guild.message = "default"
            update("level_system", "message", "guild_id", ["default", ctx.guild.id])
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMessageDefault", getLocale(self.bot, languageStrings, guildLocale, "levelsysDefaultMessage")))
        
        if len(message) > 1000:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysMessageTooLong"))
    ...
    async def levelsystem_channel(self, ctx, channel: nextcord.TextChannel = None):
        ...
        if channel is None:
            ...
            return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysChannelNone"))
    ...
    async def levelsystem_custom_add(self, ctx, level: int, *, message: str):
        ...
        if level < 2:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomAddLevel"))
        ...
        if level in guild.custom_messages:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomAddAlready", level))
        
        guild.custom_messages[level] = message
        insert("level_custommessages", "guild_id, level, message", [ctx.guild.id, level, message])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomAdded", level, message))
    ...
    async def levelsystem_custom_remove(self, ctx, level: int):
        ...
        if level not in guild.custom_messages:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomRemoveLevel", level))
        
        del guild.custom_messages[level]
        delete("level_custommessages", "guild_id level", [ctx.guild.id, level])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomRemoved", level))
    ...
    async def levelsystem_custom_show(self, ctx, level):
        if level.lower() in {"all", "a"}:
            ...
            if len(guild.custom_messages) == 0:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomShowNone"))
            ...
            return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomShowAll", custom_messages))
        
        custom_message = readOne("message", "level_custommessages", "guild_id level", [ctx.guild.id, level])
        
        if custom_message is None:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomShowLevel", level))
        
        return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysCustomShow", level, custom_message[0]))
    ...
    async def levelsystem_roles_add(self, ctx, level: int, *, role: nextcord.Role):
        ...
        if level < 2:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesAddLevel"))
        
        if guild.roles is None:
            guild.roles = {}
        
        if level in guild.roles:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesAddAlready", level))
        
        guild.roles[level] = role.id
        insert("level_roles", "guild_id, level, role_id", [ctx.guild.id, level, role.id])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesAdd", level, role.mention))
    ...
    async def levelsystem_roles_remove(self, ctx, level: int):
        ...
        if level not in guild.roles:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesRemoveLevel", level))
        
        del guild.roles[level]
        delete("level_roles", "guild_id level", [ctx.guild.id, level])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesRemoved", level))
    ...
    async def levelsystem_roles_show(self, ctx, level):
        ...
        if level.lower() in {"all", "a"}:
            if len(guild.roles) == 0:
                return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesShowNone"))
            ...
            return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesShowAll", roles))
        
        if level not in guild.roles:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesShowLevel", level))
        
        return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysRolesShow", level, ctx.guild.get_role(guild.roles[level]).mention))
    ...
    async def levelsystem_blacklist_channel_add(self, ctx, channel: nextcord.TextChannel):
        ...
        if channel.id in guild.blacklist_channel:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistAddChannelAlready", channel.mention))
        
        guild.blacklist_channel.append(channel.id)
        insert("level_blacklist_channel", "guild_id, channel_id", [ctx.guild.id, channel.id])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistAdd", channel.mention))
    ...
    async def levelsystem_blacklist_channel_remove(self, ctx, channel: nextcord.TextChannel):
        ...
        if channel.id not in guild.blacklist_channel:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistRemoveChannel"))
        
        guild.blacklist_channel.remove(channel.id)
        delete("level_blacklist_channel", "guild_id channel_id", [ctx.guild.id, channel.id])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistRemove", channel.mention))
    ...
    async def levelsystem_blacklist_role_add(self, ctx, role: nextcord.Role):
        ...
        if role.id in guild.blacklist_roles:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistAddRoleAlready"))
        
        guild.blacklist_roles.append(role.id)
        insert("level_blacklist_roles", "guild_id, role_id", [ctx.guild.id, role.id])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistAdd", role.mention))
    ...
    async def levelsystem_blacklist_role_remove(self, ctx, role: nextcord.Role):
        ...
        if role.id not in guild.blacklist_roles:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistRemoveRole"))
        
        guild.blacklist_roles.remove(role.id)
        delete("level_blacklist_roles", "guild_id role_id", [ctx.guild.id, role.id])
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistRemove", role.mention))
    ...
    async def levelsystem_blacklist_show(self, ctx):
        ...
        if not channelFieldDescription:
            channelFieldDescription = getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistShowChannelNone")

        if not roleFieldDescription:
            roleFieldDescription = getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistShowRoleNone")

        channelField = EmbedField(getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistShowChannel"), channelFieldDescription, True)
        roleField = EmbedField(getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistShowRoles"), roleFieldDescription, True)

        return await infoEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysBlacklistShowAll"), fields=[channelField, roleField])
    ...
    async def levelsystem_modify_xp_add(self, ctx, user: nextcord.Member, amount: int):
        ...
        if amount < 1:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyXpAddAmount"))
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyXpAdd", user.user_id, amount))
    ...
    async def levelsystem_modify_xp_remove(self, ctx, user: nextcord.Member, amount: int):
        ...
        if amount < 1:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyXpRemoveAmount"))
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyXpRemove", user.user_id, amount))
    ...
    async def levelsystem_modify_xp_set(self, ctx, user: nextcord.Member, amount: int):
        ...
        if amount < 1:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyXpSetAmount"))
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyXpSet", user.user_id, amount))
    ...
    async def levelsystem_modify_level_add(self, ctx, user: nextcord.Member, amount: int):
        ...
        if amount < 1:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyLevelAddAmount"))
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyLevelAdd", user.user_id, amount))
    ...
    async def levelsystem_modify_level_remove(self, ctx, user: nextcord.Member, amount: int):
        ...
        if amount < 1:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyLevelRemoveAmount"))
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyLevelRemove", user.user_id, amount))
    ...
    async def levelsystem_modify_level_set(self, ctx, user: nextcord.Member, amount: int):
        ...
        if amount < 1:
            return await errorEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyLevelSetAmount"))
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysModifyLevelSet", user.user_id, amount))
    ...
    async def levelsystem_reset_user(self, ctx, user: nextcord.Member):
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysResetUser", user.user_id))
    ...
    async def levelsystem_reset_all(self, ctx):
        ...
        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysResetAll"))
    ...
    async def levelsystem_reset_settings(self, ctx):
        ...
        update("level_system", "xp cooldown mention message channel_id", "guild_id", [3, 6, 1, getLocale(self.bot, languageStrings, guildLocale, "levelsysDefaultMessage"), "null", ctx.guild.id])

        return await successEmbed(self.bot, ctx, getLocale(self.bot, languageStrings, guildLocale, "levelsysResetSettings"))
...
def readGuild(guild: nextcord.Guild) -> Guild:
    ...
    if databaseGuild is None:
        insert("level_system", "guild_id, enabled, xp, cooldown, mention, message, channel_id", [guild.id, 0, 3, 6, 1, getLocale(self.bot, languageStrings, getGuildLanguage(guild.id), "levelsysDefaultMessage"), "null"])
        return Guild(guild.id, False, 3, 6, True, getLocale(self.bot, languageStrings, getGuildLanguage(guild), "levelsysDefaultMessage"), None, {}, {}, [], [])
...
class LevelSystemHelpView(nextcord.ui.View):
    ...
    async def calltimeout(self, bot, message):
        ...
        embed = nextcord.Embed(
                description=getLocale(self.bot, languageStrings, guildLocale, "levelsysHelp", getPrefixFromDatabase(self.bot, message)[0]),
...
class SelectorButton(nextcord.ui.Select):
    def __init__(self, bot, disabled: bool, category: str = "general", language: str = "en"):
        general = getLocale(bot, languageStrings, language, "general")
        message = getLocale(bot, languageStrings, language, "message")
        roles = getLocale(bot, languageStrings, language, "roles")
        blacklist = getLocale(bot, languageStrings, language, "blacklist")
        reset = getLocale(bot, languageStrings, language, "reset")
        ...
        super().__init__(placeholder=general, options=options, disabled=disabled)
        self.bot = bot
    ...
    async def callback(self, interaction):
        ...
        if category == getLocale(self.bot, languageStrings, guildLocale, "general").lower():
            category = "general"
        elif category == getLocale(self.bot, languageStrings, guildLocale, "message").lower():
            category = "message"
        elif category == getLocale(self.bot, languageStrings, guildLocale, "roles").lower():
            category = "roles"
        elif category == getLocale(self.bot, languageStrings, guildLocale, "blacklist").lower():
            category = "blacklist"
        elif category == getLocale(self.bot, languageStrings, guildLocale, "reset").lower():
            category = "reset"
        ...
        with contextlib.suppress(Exception):
            match category:
                case "general":
                    embed = nextcord.Embed(
                        description=getLocale(self.bot, languageStrings, guildLocale, "levelsysHelp", prefix),
                        color=nextcord.Color.blurple()
                        )

                case "message":
                    embed = nextcord.Embed(
                        description=getLocale(self.bot, languageStrings, guildLocale, "messageDescription", prefix),
                        color=nextcord.Color.blurple()
                    )

                case "roles":
                    embed = nextcord.Embed(
                        description=getLocale(self.bot, languageStrings, guildLocale, "rolesDescription", prefix),
                        color=nextcord.Color.blurple()
                    )

                case "blacklist":
                    embed = nextcord.Embed(
                        description=getLocale(self.bot, languageStrings, guildLocale, "blacklistDescription", prefix),
                        color=nextcord.Color.blurple()
                    )

                case "reset":
                    embed = nextcord.Embed(
                        description=getLocale(self.bot, languageStrings, guildLocale, "resetDescription", prefix),
                        color=nextcord.Color.blurple()
                    )
