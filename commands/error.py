...
class errorhandler(Cog):
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            await errorEmbed(self, ctx, "Dieser Command existiert nicht oder ist gerade deaktiviert.")

        elif isinstance(error, commands.BotMissingPermissions):
            await errorEmbed(self, ctx, "Dieser Bot hat nicht genug Berechtigungen.")
            ...
            await errorEmbed(f"Dir fehlen folgende Berechtigung(en) um diesen Command zu nutzen {missing}.")
            ...
            await errorEmbed(self, ctx, f"Der Command ist noch auf Cooldown, versuche es in `{error.retry_after:,.2f}` Sekunden erneut.")
