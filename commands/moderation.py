...
class moderation(Cog):
    ...
    async def _clear(self, ctx, amount: int):
        ...
        await ctx.channel.purge(limit=amount + 1, check=not messagePinned)
        await successEmbed(self.bot, ctx, f"**<:icon_moderation:967038345395961896> {amount} Nachrichten gelöscht**", delete_after=10)
    ...
    async def _kick(self, ctx, member: nextcord.Member, *, reason: str = None):
        ...
        await successEmbed(self.bot, ctx, f"**<:icon_moderation:967038345395961896> {member} wurde gekickt**", delete_after=10)
